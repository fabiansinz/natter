...
class Data:
    ...
    def __str__(self):
        ...
        if len(sh) > 1:
            s += '\t' + str(np.size(self.X,1)) + '  Examples\n'
            s += '\t' + str(np.size(self.X,0)) + '  Dimensions\n'
        else:
            s += '\t' + str(np.size(self.X,0)) + '  Examples\n'
    ...
    def norm(self,p=2.0):
        return Data(np.sum(np.abs(self.X)**p,0)**(1/p))
    ...
    def scale(self,s):
        if not (type(s) == np.ndarray): # then we assume that s is a data object
    ...
    def __getitem__(self,key):
        ...
        tmp2 = np.array(self.X[key])
        if type(key[0]) == types.IntType:
            tmp2 = np.reshape(tmp2,(1,len(tmp2)))
        elif type(key[1]) == types.IntType:
            tmp2 = np.reshape(tmp2,(len(tmp2),1))
    ...
    def makeWhiteningVolumeConserving(self,method='project',D=None):
        if method == 'project':
            if D == None:
#                F = LinearTransformFactory.DCnonDC(self)
                F = F[1:,:]
                C = np.cov((F*self).X)
                (U,D,V) = linalg.svd(C)
                D = np.array([((D[i] > 1e-8) and D[i] or 0.0) for i in range(len(D))])

            s = np.exp(-.5/len(D)*np.sum(np.log(D)))
            self.X *= s
    ...
    def dnormdx(self,p=2.0):
        ...
        for k in range(len(x)):
            x[k] = r*np.sign(x[k])*np.abs(x[k])**(p-1.0) 
        return x
