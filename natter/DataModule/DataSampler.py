...
from numpy import shape,  zeros, pi, NaN, isinf, isnan, any, array, reshape, dot,eye, ceil, arange, meshgrid,floor, sin, cos, vstack,sum,sqrt, arctan2, arange, min, max
...
def gratings(p,omega0,phi0,deltaAlpha,deltaOmega,T=1):
    ...
    G = zeros((p**2,len(deltaAlpha)*len(deltaOmega)*abs(T)))
    # will store the actual orientations and frequencies later
    labels = zeros((3,len(deltaAlpha)*len(deltaOmega)*T))
    ...
    for alpha in deltaAlpha:
        for omega in deltaOmega:
            for t in arange(T):
                G[:,k] = cos(2*pi* dot(rot(omega0*omega/p,alpha),X) + 2*pi/T *t* phi0)
                labels[0,k] = arctan2(omega0[1],omega0[0])+alpha
                labels[1,k] = f*omega
                labels[2,k] = t
                k += 1
    return Data(G,"Gratings"), labels
